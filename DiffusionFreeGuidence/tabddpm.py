...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in, num_classes, is_y_cond, window_size ,  rtdl_params={'d_layers': [32, 64, 64, 32], 'dropout': 0.0}, dim_t = 1024):
        super().__init__()
        self.dim_t = dim_t
        ...
        self.cond_up = nn.Sequential(
            nn.Linear(1,  window_size),
            nn.SiLU(),
            nn.Linear(window_size,  window_size)
        )
        
        self.diffusion_embedding=DiffusionEmbedding(max_steps=500)  ## 原始的diffwave里面代码
        self.Sinusoidal_embedding =  SinusoidalPosEmb(dim_t // 4) 
    def forward(self, x, timesteps, y=None):
        emb = self.time_embed(timestep_embedding(timesteps, self.dim_t))
        # emb =self.diffusion_embedding(timesteps) ## 原始的diffwave里面代码
        ...
        x = self.proj(x.permute(0,2,1)) + self.cond_up(emb.unsqueeze(-1)).permute(0,2,1)

        return self.mlp(x).permute(0,2,1)
