...
class Residual_group(nn.Module):
    def __init__(self, res_channels, skip_channels, num_res_layers, dilation_cycle, time_step,
        ...
        self.Sinusoidal_embedding =  SinusoidalPosEmb(diffusion_step_embed_dim_in) 
    ...
    def forward(self, input_data):
        ...
        diffusion_step_embed = swish(self.fc_t2(diffusion_step_embed)) ## 原始的SSSD里面代码

        # diffusion_step_embed =self.diffusion_embedding(diffusion_steps) ## 原始的diffwave里面代码
        # diffusion_step_embed =  self.Sinusoidal_embedding(diffusion_steps) ## DIFF-UNET里面代码
...
class DiffWaveImputer(nn.Module):
    def __init__(self, in_channels=14, res_channels=64,seq_length=48, skip_channels=64, out_channels=14, 
                 num_res_layers=5, dilation_cycle=10, time_step = 500,
...
class conditioner(nn.Module):
    ...
    def forward(self, label, cond_drop_prob=0.5):
        batch = label.shape[0]
...
if __name__ == '__main__':
    ...
    batch_size = 8
    diffwave = DiffWaveImputer(**config)
